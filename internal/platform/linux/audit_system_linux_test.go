...
func auditSystemResult() contract.Result {
	return contract.Result{ObservedAt: time.Date(2026, 9, 12, 0, 0, 0, 0, time.UTC), Data: map[string]any{"coverage_complete": true}}
}
...
func TestAuditAccountsExcludeSecretsAndPageGroups(t *testing.T) {
	...
	r = auditSystemResult()
	c.auditAccounts(context.Background(), &r, contract.Args{Offset: 2, Limit: 1})
	items := r.Data["items"].([]map[string]any)
	if len(items) != 1 || items[0]["kind"] != "group" || len(items[0]["members"].([]string)) != 2 {
		t.Fatal(r)
	}
	c.Policy.Rules = append(c.Policy.Rules, policy.Rule{Category: "files", Pattern: "/etc/passwd", Deny: true})
	r = auditSystemResult()
	c.auditAccounts(context.Background(), &r, contract.Args{Limit: 10})
	if !auditHasIssue(r, "policy_denied") || len(r.Data["items"].([]map[string]any)) != 1 {
		t.Fatal(r)
	}
}
func TestAuditStorageOmitsMountCredentialsAndMarksMalformed(t *testing.T) {
	...
	if !auditHasIssue(r, "malformed_source") || r.Data["coverage_complete"] != false {
		t.Fatal(r)
	}
	mounts := r.Data["mounts"].([]map[string]any)
	if len(mounts) != 1 || mounts[0]["mount"] != "/data set" {
		t.Fatal(r)
	}
	b, _ := json.Marshal(r)
...
func TestAuditUpdatesExpiredMetadataAndPolicy(t *testing.T) {
	...
	if !auditHasIssue(r, "stale_metadata") || len(r.Data["repository_metadata"].([]map[string]any)) != 1 {
		t.Fatal(r)
	}
	b, _ := json.Marshal(r)
	if strings.Contains(string(b), "SECRET") {
	...
	r = auditSystemResult()
	c.auditUpdates(context.Background(), &r)
	if !auditHasIssue(r, "policy_denied") || len(r.Data["repository_metadata"].([]map[string]any)) != 0 {
		t.Fatal(r)
	}
}
func TestAuditSecurityMalformedAndMissingControls(t *testing.T) {
	...
	controls := r.Data["kernel_controls"].(map[string]any)
	if controls["/proc/sys/kernel/randomize_va_space"] != uint64(2) || controls["/proc/sys/kernel/kptr_restrict"] != nil || !auditHasIssue(r, "malformed_source") || !auditHasIssue(r, "source_unavailable") {
		t.Fatal(r)
	}
}
...
func TestAuditSystemRecordAndReadBudgetLimits(t *testing.T) {
	...
	r = auditSystemResult()
	c.auditStorage(context.Background(), &r)
	if !r.Truncated || len(r.Data["devices"].([]map[string]any)) != auditMaxEntries {
	...
	r = auditSystemResult()
	c.auditAccounts(context.Background(), &r, contract.Args{Limit: 10})
	if !r.Error || !r.Truncated || !auditHasIssue(r, "inspection_limit") || len(r.Data["items"].([]map[string]any)) != 0 {
