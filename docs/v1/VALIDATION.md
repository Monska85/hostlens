...
The typed-contract change rewrote all 27 MCP tool result payloads from untyped maps to one typed Go struct per tool, derived every advertised `inputSchema` and `outputSchema` from those types, and made the gateway validate arguments and results in both directions. A snapshot test generates `docs/v1/tools.json` from a diagnostics-role `tools/list` call and fails on any description, member, or schema drift; the file ships in every archive and as a standalone checksummed release asset. A scripted member-name comparison against the pre-refactor wire capture found and fixed two wire regressions (`get_os_info`/`get_inventory` `kernel` and `get_docker_disk_usage` `reclaimable` had wrongly become optional) and confirmed the remaining optional-member differences as honest failure-path modeling; the conclusions are recorded in the change's design notes.
