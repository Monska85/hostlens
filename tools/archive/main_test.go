...
func TestReleaseMaterials(t *testing.T) {
	for _, missing := range []string{"", "NOTICE.txt", "licenses/dependency_LICENSE", "version"} {
		t.Run(missing, func(t *testing.T) {
			...
			for _, name := range []string{"hostlens", "hostlens-diagnostics", "LICENSE", "NOTICE.txt", "config.example.yaml", "OPERATIONS.md", "VALIDATION.md", "profiles/nginx.yaml", "profiles/allow-all.yaml", "licenses/dependency_LICENSE", "release.json"} {
				if name == missing {
					continue
				}
				data := []byte("fixture")
